        ...
        <ul className="mobile:mt-10 tablet:mt-0">
          {serviceList.map(({ link, title, subtitle, description, iconComponent }, index) => (
            <li key={index} className={`${index === 0 ? "mobile:pb-5 tablet:pb-10" : "border-t-[1px] border-gray-300 mobile:py-5 tablet:py-10"}`}>
              <a href={link} target="_blank" rel="noopener noreferrer" className="group grid mobile:grid-cols-[min-content_1fr] tablet:grid-cols-[min-content_1fr_min-content] gap-x-5 gap-y-1">
                ...
                <span className="tablet:col-[3/4] tablet:row-[1/3] self-center w-10 h-10 flex items-center justify-center rounded-full group-hover:bg-gray-300 transition-all duration-500">
                  <HiArrowRight />
                </span>
              </a>
            </li>
